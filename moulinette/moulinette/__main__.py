...
from moulinette.extract_functions_infos import (
...
class Moulinette:
    ...
    def prepare_exercises(
        ...
        if set not in ("public", "private"):
            self.output.error(
                f"Invalid set '{set}'. Must be 'public' or 'private'."
            )
            return
        ...
        for i, (student_answer, correction) in enumerate(
            ...
            try:
                ...
                self.output.expected(
                    "Expected parameters", correction["parameters"]
                )
            ...
            if student_output != correction["expected_output"]:
                ...
                self.output.expected(
                    "Expected call",
                    f"{correction['name']}({correction['parameters']})",
                )
                self.output.expected(
                    "Expected result", correction["expected_output"]
                )
